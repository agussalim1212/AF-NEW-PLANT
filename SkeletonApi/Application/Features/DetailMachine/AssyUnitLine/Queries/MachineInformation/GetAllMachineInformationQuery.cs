...
using SkeletonApi.Application.Features.DetailMachine.AssyWheelLine.TotalProductionAssyWheel;
...
{
    public class GetAllMachineInformationQuery : IRequest<Result<GetAllMachineInformationDto>>
    {
            ...
            public async Task<Result<GetAllMachineInformationDto>> Handle(GetAllMachineInformationQuery query, CancellationToken cancellationToken)
        {
            var machine = await _unitOfWork.Repo<SubjectHasMachine>().Entities.Include(s => s.Machine).Include(s => s.Subject)
                          ...
                          || (query.MachineId == m.MachineId && m.Subject.Vid.Contains("CALIBRATION"))).ToListAsync();
            ...
            IEnumerable<string> vids = machine.Select(m => m.Subject.Vid).ToList();
                ...
                var categorys = await _unitOfWork.Data<Dummy>().Entities
                  ...
                  .Select(groups => new
                  {
                      ...
                      LastKalibrasi = groups.Where(g => g.Id.Contains("CALIBRATION"))
                          .OrderByDescending(g => g.DateTime)
                          .FirstOrDefault(), // Get the last "Cycle-Count" element
            ...
            else
            {
                data = new GetAllMachineInformationDto
                {
                    ...
                    ValueRunning = categorys.Select(x => x.LastRunTime?.Value).FirstOrDefault(),
                    CycleCount = categorys.Select(c => c.LastCycleCount?.Value).Skip(1).FirstOrDefault(),
